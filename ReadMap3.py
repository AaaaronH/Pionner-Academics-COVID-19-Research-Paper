...
fout = open("BerkeleyLatLonSmall.txt", "w")
...
osmHandler.apply_file("Berkeley.osm")
    ...
    for tag in nodesTags[id]:
        ...
        for t in tag:
            ...
            if 'addr' in t:
                addr = True
    if house and addr:
        houseIndex.append(nodesTags[id][0][2])

fout.write(str(len(houseIndex)) + '\n')
for i in houseIndex:
    ans = str(i.lat) + ' ' + str(i.lon) + '\n'
    fout.write(ans)
